function t396_init_hook(recid) {
	// var record = document.getElementById('rec' + recid);
  var record = document.querySelector('.rec' + recid);
	...
	if (!isTildaModeEdit && t396_ab__getFieldValue(artBoard, 'fixed-need-js') === 'y') {
...
function t396__initOnlyScalable() {
	...
	window.shouldUseScaleFactor = !window.isOnlyScalable && !firefoxMatch && chromeVersion <= 127;
}
...
function t396__setGlobalScaleVariables(recid, resolution, isScaled) {
	...
	if (isScaled) {
		window.tn[artBoardId].scaleFactor = parseFloat((window.tn_window_width / resolution).toFixed(3));
...
function t396_doResize(recid) {
	...
	if (!isOnlyScalable) {
		...
		Array.prototype.forEach.call(coreElementList, function (coreElement) {
			 ...
			 */
			if (coreElement.classList.contains('tn-atom') && coreElement.closest('.tn-molecule')) return;
	...
	if (tildaMode !== 'edit' && t396_ab__getFieldValue(artBoard, 'fixed-need-js') === 'y') {
		t_onFuncLoad('t396__processFixedArtBoard', function () {
			// eslint-disable-next-line no-undef
			t396__processFixedArtBoard(artBoard);
		});
	...
	var wrappers = [artBoard, artBoard.querySelector('.t396__carrier'), artBoard.querySelector('.t396__filter')];
...
function t396__processAbsoluteArtBoard(artBoard) {
	...
	var filterHasNotBG = filter ? getComputedStyle(filter).backgroundImage === 'none' : true;
	var abHasNotBG = abStyles.backgroundColor === 'rgba(0, 0, 0, 0)' && abStyles.backgroundImage === 'none';
...
function t396_detectResolution(recid) {
	...
	if (typeof resolution === 'undefined') resolution = window.tn[artBoardId].screens[0];
...
function t396__getAxisXPadding() {
	var allRecords = document.getElementById('allrecords');
	var paddingChecker = Boolean(window.tildaMembers) || window.zero_window_width_hook === 'allrecords';
...
function t396_artboard_build(data, recid) {
	...
	if ((artBoardOverflow === 'visible' || artBoardOverflow === 'visibleX') && allRecords) {
		...
		var sidebarSticky = document.querySelector('.t951__sidebar_sticky,.t-store__prod-popup__col_fixed');
		if (sidebarSticky) {
...
function t396__getCurrentScaleFactor(recid) {
	var artBoardId = 'ab' + recid;
	var abScaleFactor = window.tn && window.tn[artBoardId] && window.tn[artBoardId].scaleFactor;
...
function t396_processElemTransform(element) {
	...
	var elementHasBlur =
		...
		(elementStyles.webkitBackdropFilter && elementStyles.webkitBackdropFilter !== 'none');
...
function t396__processBackdropFilterOnImage(elem) {
	...
	var hasExtendedAnimation = elem.getAttribute('data-animate-prx') || elem.getAttribute('data-animate-fix');
	var hasAnimation = hasSBS || hasExtendedAnimation;
	var isProcessed = elem.classList.contains('t396__elem--backdrop-filter-img-wrappered');
...
function t396__updateBackdropFilterOnImage(elem) {
	...
	var animWrapper = elem.querySelector('.tn-atom__sbs-anim-wrapper, .tn-atom__prx-wrapper, .tn-atom__sticky-wrapper');
	var targetFilterValue = '';
	if (animWrapper) {
		elem = animWrapper;
		targetFilterValue = window.getComputedStyle(elem).webkitBackdropFilter || '';
	}
...
function t396_elem__getFieldValue(element, prop) {
	...
	if (curRes === curResMax) {
		...
		dataField = element.getAttribute('data-field-' + prop + '-res-' + curRes + '-value');
	}
	...
	if (!dataField && dataField !== '') {
		for (var i = 0; i < screens.length; i++) {
			...
			if (screen === curResMax) {
				...
				dataField = element.getAttribute('data-field-' + prop + '-res-' + screen + '-value');
			}
...
function t396_group__renderView(group) {
	...
	fields.forEach(function (field) {
		...
		switch (field) {
			case 'left':
				value = t396_elem__convertPosition__Local__toAbsolute(group, field, value);
...
function t396_elem__renderViewOneField(element, field) {
	...
	var artBoard = element.closest('.t396__artboard');
	var isScaled = t396_ab__getFieldValue(artBoard, 'upscale') === 'window';
	if (element.getAttribute('data-scale-off') === 'yes' && isScaled && tildaMode !== 'edit') {
		return;
	}
...
function t396_elem__convertPosition__Local__toAbsolute(element, field, value) {
	...
	var isGroupPhysical =
		element.classList.contains('tn-group') && t396_group__getFieldValue(element, 'type') === 'physical';
	var parentGroup = element.parentNode.closest('.tn-group');
	var isParentGroupPhysical = t396_group__getFieldValue(parentGroup, 'type') === 'physical';
	...
	switch (field) {
		case 'left':
			...
			if (isParentGroupPhysical) {
				...
				var parentGroupLeftUnits = t396_group__getFieldValue(parentGroup, 'leftunits');

				if (parentGroupLeftUnits === '%') {
					parentGroupLeft = t396_roundFloat((elementContainerWidth * parentGroupLeft) / 100);
				}
			...
			if (!isTildaModeEdit && isScaled) {
				if (container === 'grid' && isOnlyScalable) value = value * currentScaleFactor;
			} else {
		...
		case 'top':
			...
			if (isParentGroupPhysical) {
				...
				var parentGroupTopUnits = t396_group__getFieldValue(parentGroup, 'topunits');

				if (parentGroupTopUnits === '%') {
					parentGroupTop = t396_roundFloat((elementContainerHeight * parentGroupTop) / 100);
				}
			...
			if (isZoomedElement && elementContainer !== 'window') {
				offsetTop = verticalAlignValue === 'stretch' ? 0 : offsetTop / currentScaleFactor;
			}
			...
			if (isScaledElement && elementContainer !== 'window') {
				elementHeight = getElementHeight(element);
				var scaledDifference = (elementHeight * currentScaleFactor - elementHeight) / 2;
...
function t396_ab__getFieldValue(artBoard, prop) {
	...
	if (curRes === curResMax) {
		...
		dataField = artBoard.getAttribute('data-artboard-' + prop + '-res-' + curRes);
	}
	...
	if (dataField === null) {
		for (var i = 0; i < screens.length; i++) {
			...
			if (screen === curResMax) {
				...
				dataField = artBoard.getAttribute('data-artboard-' + prop + '-res-' + screen);
			}
...
function t396_group__getFieldValue(group, field) {
	...
	var dataField;
	var recid = group.closest('.t396__artboard').getAttribute('data-artboard-recid');
	var artBoardId = 'ab' + recid;

	var curRes = window.tn[artBoardId].curResolution;
	var curResMax = window.tn[artBoardId].curResolution_max;
	var screens = window.tn[artBoardId].screens;
	var postfix = ['widthmode', 'heightmode', 'flex'].includes(field) ? '' : '-value';

	if (curRes === curResMax) {
		dataField = group.getAttribute('data-group-' + field + postfix);
	} else {
		dataField = group.getAttribute('data-group-' + field + '-res-' + curRes + postfix);
	}
	...
	if (dataField === null) {
		for (var i = 0; i < screens.length; i++) {
			...
			if (screen === curResMax) {
				...
				dataField = group.getAttribute('data-group-' + field + '-res-' + screen + '-value');
			}
